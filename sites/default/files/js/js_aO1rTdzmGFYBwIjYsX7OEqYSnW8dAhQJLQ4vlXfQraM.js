...
(function ($) {
  Drupal.behaviors.video_playlist = {
    attach: function(context, settings) {
      var playlist_count = settings.video_playlist.playlist_count;
      var element_settings = [];
      playlist_clear_clone_ids();
      for (var i = 0; i < playlist_count; i++) {
        // Find the current tid value, which can be used to tie into the
        // buttons and playlist display areas.
        if (settings.video_playlist.playlists[i].tid != 'undefined') {
          var cur_tid = settings.video_playlist.playlists[i].tid;
          var cur_offset = settings.video_playlist.playlists[i].use_more.offset;
          settings.video_playlist.playlists[cur_tid]=settings.video_playlist.playlists[i];

          // Setup click events for the load more buttons.
          element_settings = {
            url: Drupal.settings.basePath + 'fetchplaylist',
            event: 'click',
            tid: cur_tid,
            offset: cur_offset,
            progress: {
            ...
            beforeSerialize: function(element, options) {
              options.data['tid'] = $(this)[0].tid;
              options.data['offset'] = $(this)[0].offset;

              return Drupal.ajax.prototype.beforeSerialize(element, options);
            },
            beforeSend: function() {
              $('#' + this.element.id + ':after').css('display','none');
              this.origText = $(this.element).html();
              $(this.element).html(Drupal.t('loading...'));
            },
            error: function() {
              $(this.element).html(this.origText);
            },
            success: function (data) {
              // Load the data.
              playlist_show_more(data);

              // Hang on to the data for later use instead of reloading via ajax.
              settings.video_playlist.playlists[data.tid].cache = data;
              $('#playlist-more-button-' + data.tid).unbind('click');
              $('#playlist-more-button-' + data.tid).click(function(){
                  playlist_show_more(settings.video_playlist.playlists[data.tid].cache);
              });
            }
          };

          Drupal.ajax['playlist_more_' + cur_tid] = new Drupal.ajax('playlist_more_' + cur_tid, $('#playlist-more-button-' + cur_tid), element_settings);
        }
      }
      ...
      $('.video-playlist').once('video-playlist')
          .flexslider({
            ...
            before: function (slider) {
              Drupal.moveMarker(slider.animatingTo);
            },
            start: function (slider) {
              ...
              setTimeout(function () {
                if ($('.video-playlist .playlist-display li').length > 2) {
                  ...
                  var flexHeight = parseInt(curSlide.height(), 10) + parseInt(curSlide.css('padding-bottom'), 10);
                  $('.video-playlist .flex-viewport').animate({height: flexHeight});
                }
              }, 200);
            }
          });
      ...
      $('.playlist-menu li a').each(function () {
        ...
        $(this).bind('click', function () {
          ...
          if (playpadding !== null) {
            ...
            flexHeight = playheight + playpadding;
          } else {
            flexHeight = playheight;
          }
          setTimeout(function () {
            $('.video-playlist .flex-viewport').animate({height: flexHeight});
          }, 200);
      ...
      /**
       * Pulled the display of the additional data out into a function so that it can be called
       * after ajax returns data or if local cached data is being re-used.
       */
      function playlist_show_more(data) {
        // Clear out id's on the left and right clones generated by the slideshow.
        playlist_clear_clone_ids();

        $('#playlist-more-' + data.tid).html(data.content);
        setTimeout(function(){
          $('.video-playlist .flex-viewport').animate({height: parseInt($('#playlist-display-' + data.tid).height(), 10) + parseInt($('#playlist-display-' + data.tid).css('padding-bottom'), 10)});
        },200);
        $('#playlist-more-button-' + data.tid).hide();
        $('#playlist-more-button-' + data.tid).html(Drupal.t(settings.video_playlist.playlists[data.tid].use_more.use_more_text));
        $('#playlist-less-button-' + data.tid).css('display','inline-block');
        $('#playlist-less-button-' + data.tid).click(function(){
          diff = $('.flex-viewport').height();
          $('#playlist-more-' + data.tid).html('');
          offset = $('#playlist-display-' + data.tid).offset();
          setTimeout(function(){
            $('html, body').animate({scrollTop: offset.top });
            $('.video-playlist .flex-viewport').animate({height: parseInt($('#playlist-display-' + data.tid).height(), 10) + parseInt($('#playlist-display-' + data.tid).css('padding-bottom'), 10)});
          },200);
          $('#playlist-more-button-' + data.tid).show();
          $('#playlist-less-button-' + data.tid).hide();
        });
      }

      /**
       * Makes sure that all clones created by the slideshow get the ID's removed.
       */
      function playlist_clear_clone_ids() {
        $('.playlist-display .clone').attr('id', '');
        $('.playlist-display .clone .playlist-more').attr('id', '');
        $('.playlist-display .clone .playlist-more-button').attr('id', '');
        $('.playlist-display .clone .playlist-less-button').attr('id', '');
      }
...
;
(function ($) {
  Drupal.behaviors.all_videos = {
    attach: function(context, settings) {
      var element_settings = [];
      // Set the intial active classes on the All options.
      all_videos_set_active_filter('all');
      all_videos_set_active_node_filter('all');

      // Set up click events for filter buttons.
      var filter_count = settings.all_videos.playlist.filters.length;
      for (var i = 0; i < filter_count; i++) {
        var cur_tid = settings.all_videos.playlist.filters[i].tid;

        // Alter the default click action of each term button. (categories)
        $('#all-videos-filter-button-' + cur_tid).bind('click', { tid: cur_tid }, function(e){
          e.preventDefault();
          if (!$(this).hasClass('active')) {
            // Update the cur_tid setting and refresh the filtering.
            settings.all_videos.playlist.cur_filter = parseInt(e.data.tid, 10);
            all_videos_update_filtering($(this));
            all_videos_set_active_filter(e.data.tid);
          }
        });
      }

      // Disable default behavior on node filter drop trigger.
      $('.pane-all-videos-playlist .node-filter-drop .drop-trigger').bind('click', function(e){
        e.preventDefault();
      });
      // Disable default behavior on term filter drop trigger.
      $('.pane-all-videos-playlist .term-filter-drop .drop-trigger').bind('click', function(e){
        e.preventDefault();
      });

      // Set up click events for the node filter buttons. (characters)
      var node_filter_count = settings.all_videos.playlist.node_filters.length;
      for (var i = 0; i < node_filter_count; i++) {
        var cur_nid = settings.all_videos.playlist.node_filters[i].nid;

        // Alter the default click action of each term button.
        $('#all-videos-node-filter-button-' + cur_nid).bind('click', { nid: cur_nid }, function(e){
          e.preventDefault();

          // Update the cur_tid setting and refresh the filtering.
          settings.all_videos.playlist.cur_node_filter = parseInt(e.data.nid, 10);
          all_videos_update_filtering($(this));
          all_videos_set_active_node_filter(e.data.nid);
        });
      }
      $('.node-filter.node-filter-drop .drop-trigger').bind('mouseenter', function(){
        $('.node-filter.node-filter-drop').addClass('hover');
      }).bind('mouseleave', function(){
        $('.node-filter.node-filter-drop').removeClass('hover');
      }).bind('click', function(){
        $('.node-filter.node-filter-drop').toggleClass('hover');
      });
      $('.node-filter.node-filter-drop ul').bind('mouseenter', function(){
        $('.node-filter.node-filter-drop').addClass('hover');
      }).bind('mouseleave', function(){
        $('.node-filter.node-filter-drop').removeClass('hover');
      });
      
      $('.node-filter.term-filter-drop .drop-trigger').bind('mouseenter', function(){
        $('.node-filter.term-filter-drop').addClass('hover');
      }).bind('mouseleave', function(){
        $('.node-filter.term-filter-drop').removeClass('hover');
      }).bind('click', function(){
        $('.node-filter.term-filter-drop').toggleClass('hover');
      });
      $('.node-filter.term-filter-drop ul').bind('mouseenter', function(){
        $('.node-filter.term-filter-drop').addClass('hover');
      }).bind('mouseleave', function(){
        $('.node-filter.term-filter-drop').removeClass('hover');
      });
      ...
      $('#all-videos-more-button').addClass('button-style arrow').bind('click', function(e){
        e.preventDefault();

        all_videos_load_more($(this));
      }).wrap('<div class="button-container">');

      // Check that the load more button is needed.
      all_videos_toggle_load_more_button();

      /**
       * Ajax call to update the main all videos content area based on a filter settings.
       */
      function all_videos_update_filtering(button) {
        $.ajax({
          url: Drupal.settings.basePath + 'filter_videos',
          type: 'POST',
          data: {
            filter : parseInt(settings.all_videos.playlist.cur_filter, 10),
            node_filter : parseInt(settings.all_videos.playlist.cur_node_filter, 10)
          },
          dataType : 'json',
          beforeSend: function() {
            // Hold on to the original text and show loading.
            this.origText = button.html();
            button.html(Drupal.t('loading...'));
          },
          success: function(data) {
            // Swap content and clear out the load more area.
            $('#all-videos-main').html(data.rendered);
            $('#all-video-load-more-area').html('');
            $('.node-filter').removeClass('hover');
            
            // Restore original button state.
            button.html(this.origText);

            // Update the settings based on returned data.
            settings.all_videos.playlist.use_more.more_avail = data.use_more.more_avail;
            settings.all_videos.playlist.use_more.offset = parseInt(data.use_more.offset, 10);
            settings.all_videos.playlist.use_more.total_rows = parseInt(data.use_more.total_rows, 10);

            // Determine if the load more button is still needed.
            all_videos_toggle_load_more_button();
          }
        });
      }
      ...
      /**
       * Ajax call to load more videos into the load more area.
       */
      function all_videos_load_more(button) {
        // Make sure loading more is even available.
        if (settings.all_videos.playlist.use_more.more_avail === true) {
          $.ajax({
            url: Drupal.settings.basePath + 'fetch_more_videos',
            type: 'POST',
            data: {
              filter : parseInt(settings.all_videos.playlist.cur_filter, 10),
              node_filter : parseInt(settings.all_videos.playlist.cur_node_filter, 10),
              offset : parseInt(settings.all_videos.playlist.use_more.offset, 10),
              items_per_page : 24,  // Number of videos to return. Todo, would be nice to make this a setting.
              total_rows : parseInt(settings.all_videos.playlist.use_more.total_rows, 10)
            },
            dataType : 'json',
            beforeSend: function() {
              // Hold on to the original text and show loading.
            ...
            success: function(data) {
              // Append content to the end of the load more area.
              var more_area_contents = $('#all-video-load-more-area').html();
              more_area_contents = more_area_contents + data.rendered;
              $('#all-video-load-more-area').html(more_area_contents);
              ...
              // Update the settings based on returned data.
              settings.all_videos.playlist.use_more.more_avail = data.use_more.more_avail;
              settings.all_videos.playlist.use_more.offset = data.use_more.offset;
              settings.all_videos.playlist.use_more.total_rows = parseInt(data.use_more.total_rows, 10);
      ...
      /**
       * Toggle active state on the filters buttons.
       */
      function all_videos_set_active_filter(tid) {
        $('#all-videos-filters li a').each(function() {
          $(this).removeClass('active');
        });
        $('#all-videos-filter-button-' + tid).addClass('active');
      }

      /**
       * Toggle active state on the node filters buttons.
       */
      function all_videos_set_active_node_filter(nid) {
        $('#all-videos-node-filters li a').each(function() {
          $(this).removeClass('active');
        });
        $('#all-videos-node-filter-button-' + nid).addClass('active');
      }

      /**
       * Toggle visibility of the load more button.
       */
      function all_videos_toggle_load_more_button() {
        if (settings.all_videos.playlist.use_more.more_avail === true) {
          $('#all-videos-more-button').show();
        }
        else {
          $('#all-videos-more-button').hide();
        }
      }
    }
  };
