...
(function($) {
  ...
  enquire.register(
    'screen and (max-width: 780px) and (-webkit-device-pixel-ratio:2), screen and (max-width: 780px) and (device-pixel-ratio:2), screen and (max-width: 780px) and (-webkit-device-pixel-ratio:3), screen and (max-width: 780px)',
    function() {
      Drupal.behaviors.mobileEtc = {
        attach: function(context) {
          ...
          $('#footer .custom-social-widgets .social-section', context).on(
            'touchend',
            function(e) {
              if ($(this).hasClass('open')) {
                $(this).removeClass('open');
              } else {
                $('#footer .custom-social-widgets .social-section').removeClass(
                  'open'
                );
                $(this).addClass('open');
              }
      ...
      Drupal.behaviors.initiScroll = {
        attach: function(context) {
          ...
          $('.start-here .pane-content  .view-content').once(
            'wrapper',
            function() {
          ...
          var initScroll = function() {
            ...
            var els = [
              ...
              '.imprint-must-read .view-content',
            ...
            setTimeout(function() {
              ...
              var promo_width =
                134 *
                  $('.marquee-promos.processed .view-content .views-row')
                    .length +
                10;
              $('.marquee-promos.processed .view-content').css(
                'width',
                promo_width + 'px'
              );
              ...
              for (var i = 0; i < els.length; i++) {
                if ($(els[i]).length > 0) {
                  ...
                  if (
                    els[i] === '.pane-must-list .view-content' ||
                    els[i] === '.imprint-must-read .view-content'
                  ) {
          ...
          if (
            $(
              '#block-views-three-image-marquee-block .view-three-image-marquee.view-display-id-block'
            )
          ) {
      ...
      Drupal.behaviors.mobileMenuInteraction = {
        attach: function(context) {
          ...
          $('.mobile-menu').once(function() {
            ...
            $('#block-menu-menu-dc-menu--2 > .block-inner > .content > .menu').before('<a class="menu-title">Menu</a>');
            var all_sites = '<a class="menu-title">All Sites</a>';
            all_sites += $('#block-system-main-menu--2 .content').html();
            var dc_ent = '<a class="menu-title">DC Entertainment Info</a>';
            dc_ent += $('#block-menu-menu-footer-dc .content').html();
            $(
          ...
          $('.mobile-trigger', context).on('click tap touch', function(e) {
            e.preventDefault();
            if (!$(this).next('.mobile-sub').hasClass('open')) {

              if ($(this).hasClass('menu-trigger')) {
                // Set height of open menu. This allows scrolling within the menu.
                var menu_height = $(window).height() - $('#header-mobile').height();
                $('.mobile-menu').height(menu_height);
          ...
          $('a.close').click(function() {
            if (
              ...
              $('.mobile-trigger, .mobile-sub, .mobile-menu li').removeClass(
                'open'
              );
              $('#header').removeClass('paused');
            }
          ...
          // Submenu behaviors
          $('.mobile-menu li.expanded a:first-child', context).click(function(
            e
          ) {
            if (
              !$(this)
  ...
  Drupal.behaviors.rebirthFloodlightTags = {
    attach: function(context) {
      $('#dce-fl-rebirth').click(function() {
        ...
        $(this).append(
          '<iframe src="https://3461539.fls.doubleclick.net/activityi;src=3461539;type=dcreb0;cat=dcreb00;dc_lat=;dc_rdid=;tag_for_child_directed_treatment=;ord=1;num=' +
            a +
    ...
    attach: function(context) {
      if (!$('#comics-carousel-jumper').length) {
        $('#quicktabs-comic_carousel_landing_2016 ul').append(
          '<li id="comics-carousel-jumper"><a href="#">See All ></a><li>'
        );
      }
    }
  };
    ...
    attach: function(context) {
      if (!$('#collectibles-carousel-jumper').length) {
        $('#quicktabs-collectibles_landing_carousel ul').append(
          '<li id="collectibles-carousel-jumper" style="color: white;"><a href="#">See All ></a><li>'
        );
      }
    }
  };
  ...
  Drupal.behaviors.fadeableMenu = {
    attach: function(context) {
      ...
      var headerPersistent =
        $(window).width() > 780
          ? $('#header-persistent', context)
      ...
      if (headerPersistent.length > 0) {
        ...
        fadeableWindow
          .off('scroll.fadeable')
          .on('scroll.fadeable', function(event) {
            didScroll = true;
          });
        ...
        function hasScrolled() {
          ...
          if (st < lastScrollTop && st <= navbarPosition) {
            ...
            if (st > lastScrollTop && st > navbarPosition + navbarHeight) {
              ...
              // Scroll Up
              if (
                st > navbarPosition + navbarHeight &&
                st + fadeableWindow.height() < fadeableDocument.height()
              ) {
                headerPersistent
                  .removeClass('nav-up')
                  .addClass('nav-sticky nav-down');
              }
            }
          }
  ...
  // dc_comics_essentials view horizontal scrolling displays.
  Drupal.behaviors.essentialsViewScrollable = {
    attach: function(context) {
      if ($('.view-dc-comics-essentials.style--scrollable').length === 0) {
        return;
      }

      $('.view-dc-comics-essentials.style--scrollable').each(function() {
        // Establish some vars
        var $view = $(this, context);
        var $slider = $view.find('.view-content', context);
        var $pagerPrev = $view.find('.pager-prev');
        var $pagerNext = $view.find('.pager-next');
        var isSliding = false;
        var sliderWidth = $slider.width();
        var sliderAnimDurr = 700;
        var sliderCurrPos = 0;
        var sliderPrevPos = 0;
        var $rightMostVisSlide = getRightMostVisSlide();
        var slidesTotalWidth = getTotalSlidesWidth();
        var sliderEndPos = slidesTotalWidth - sliderWidth;

        $pagerPrev.hide();

        $pagerPrev.on('click tap touch', function() {
          if ($slider.hasClass('animating')) {
            return;
          }
          $slider.addClass('animating');

          if (sliderPrevPos < sliderCurrPos) {
            // $slider.animate({scrollLeft: sliderPrevPos}, sliderAnimDurr);
            $slider[0].scrollLeft = sliderPrevPos;
          }
          else {
            // $slider.animate({scrollLeft: sliderCurrPos - sliderWidth}, sliderAnimDurr);
            $slider[0].scrollLeft = sliderCurrPos - sliderWidth;
          }

          setTimeout(function() {
            $slider.removeClass('animating');
            $rightMostVisSlide = getRightMostVisSlide();
          }, sliderAnimDurr);
        });

        $pagerNext.on('click tap touch', function() {
          if ($slider.hasClass('animating')) {
            return;
          }
          $slider.addClass('animating');

          sliderPrevPos = $slider.scrollLeft();

          var slideRect = $rightMostVisSlide.next()[0].getBoundingClientRect();
          // $slider.animate({scrollLeft: slideRect.x + sliderCurrPos}, sliderAnimDurr);
          $slider[0].scrollLeft = slideRect.x + sliderCurrPos;
        ...
        $slider.on('scroll', function() {
          sliderCurrPos = $slider.scrollLeft();

          if (sliderCurrPos >= sliderEndPos - 50) {
            $pagerNext.hide();
          }
          else {
            $pagerNext.show();
        ...
        function getRightMostVisSlide() {
          var $rightMostVisSlide;

          $slider.find('.essential-slide').each(function(index) {
            var slideOffset = $(this).position().left + $(this).width();

            if (slideOffset < sliderWidth) {
              $rightMostVisSlide = $(this);
            }
          });

          return $rightMostVisSlide;
        }
        ...
        $slider.find('.views-row.essential-card').on(
          'click tap touch',
          function() {
            if (isSliding) {
              return;
        ...
        // Close meta overlay
        var close = function() {
          $slider.find('.essential-meta.active .close').on('click tap touch', function(e) {
            $slider.find('.views-row').removeClass('active');
            $(this)
              .parent()
              .remove();
          });
        ...
        var slideIn = function(item) {
          ...
          $(meta)
            ...
            .addClass('animated');
          setTimeout(function() {
            $slider.find('.essential-meta.animated').addClass('active');
        ...
        var paging = function() {
          ...
          var nextCard;
          // Prev functionality
          $slider.find('.essential-meta.active .prev').on('click tap touch', function() {
            nextCard = $slider.find('.essential-card.active').prev();
            page(nextCard);
            // Call function on itself to re-init bindings
            paging();
          });
          // Next functionality
          $slider.find('.essential-meta.active .next').on('click tap touch', function() {
            nextCard = $slider.find('.essential-card.active').next();
            page(nextCard);
            // Call function on itself to re-init bindings
            paging();
          });
        ...
        var page = function(nextCard) {
          ...
          $slider.find('.views-row').removeClass('active');
          $slider.find('.essential-meta.active').addClass('slideOut');
          setTimeout(function() {
            $slider.find('.essential-meta.slideOut').remove();
          }, 1000);
  ...
  Drupal.behaviors.youngAnimalFloodlightTags = {
    attach: function(context) {
      if (
        window.location.pathname === '/comics/doom-patrol-2016/doom-patrol-1'
      ) {
        $('.buy-now-tab').click(function() {
          var axel = Math.random() + '';
          var a = axel * 10000000000000;
          $(this).append(
            '<iframe src="https://3461539.fls.doubleclick.net/activityi;src=3461539;type=dcyou00;cat=dcyou00;dc_lat=;dc_rdid=;tag_for_child_directed_treatment=;ord=1;num=' +
              a +
              '?" width="1" height="1" frameborder="0" style="display:none"></iframe>'
          );
        });
  ...
  Drupal.behaviors.pullList = {
    attach: function(context) {
      ...
      originalTitle.html(
        originalTitle.html().replace(/^(\w+)/, '<span>$1</span>')
      );
    }
  };

  // Attaching new behavior to the Comics page quicktab links. Adding a loading indicator.
  Drupal.behaviors.comicsQuicktabsClick = {
    attach: function(context) {
      $('#quicktabs-comic_carousel_landing_2016 .quicktabs-tabs a')
        .once('comicsQuicktabsClick')
        .on('click', function(e) {
          var page_id =
            '#' +
            $(this)
          ...
          $(this)
            .parents('.item-list')
            .append(
              '<div id="comics-loading" class="loading-indicator">Please Wait...</div>'
            );
        });
    }
  };

  // Cleaning up comics page loading indicator once ajax call is completed.
  Drupal.behaviors.comicsMarqueeLoadingIcon = {
    attach: function() {
      var loading = $('#comics-loading');
      var comicsMarquee = $('#quicktabs-comic_carousel_landing_2016');
      if (comicsMarquee.length > 0) {
        comicsMarquee.imagesLoaded(function(a) {
          if (loading.length > 0) {
            loading.remove();
          }
        });
  ...
  // Initiate Owl Carousel on Related Content view.
  Drupal.behaviors.relatedContentCarousel = {
    attach: function() {
      if ($('.view-related-content').length) {
        $('.view-related-content .owl-carousel').owlCarousel({
          loop: false,
          margin: 15,
          nav: true,
          dots: false,
  ...
  // Initiate Owl Carousel on Featured Content view.
  Drupal.behaviors.featuredContentCarousel = {
    attach: function() {
      if ($('.view-featured-content').length) {
        $('.view-featured-content .owl-carousel').owlCarousel({
          loop: false,
          margin: 15,
          nav: true,
          dots: false,
  ...
  $(window).load(function() {
    ...
    // Comics page only
    if ($('body').hasClass('page-comics')) {
      sneakPeekSlickSlider();
    }
  ...
  // Applying imagesLoaded to the slider to remove loading icon and display
  // content. Not a clean way to accomplish this, but it does the job.
  var sneakPeekSlickSlider = function() {
    var sneakPeekSlider = $('.view-dc-comics-sneak-peeks');
    sneakPeekSlider.imagesLoaded(function(a) {
      sneakPeekSlider.find('.loading').remove();
      sneakPeekSlider.find('.view-content').removeClass('invisible');
    });
  };
  ...
  var imprintNewsGrid = function() {
    var newsSection = $('.page-taxonomy-term').find('.news-section'),
      newsContent = newsSection.find('.view-content');
  ...
  var dcNewsGrid = function() {
    var wrapper = $('.page-news .view-dc-news'),
      content = wrapper.find('.view-content');
    ...
    enquire.register('screen and (min-width: 781px)', function() {
      var settings = {
        ...
        columnWidth: 404,
        gutter: 20,
        stamp: '.views-row-first'
      };

      content.imagesLoaded(function() {
        // init masonry
        content.masonry(settings);

        // removing loading spinner and show view.
    ...
    enquire.register('screen and (max-width: 780px)', function() {
      var wrapper = $('.page-news .view-dc-news'),
        ...
        columnWidth = (containerWidth - 10) / 2, // Parent minus gutter, split into two columns
        settings = {
          itemSelector: '.views-row',
          columnWidth: columnWidth,
          gutter: 10,
      ...
      content.imagesLoaded(function() {
        // On mobile, the field_slideshow js is triggered before the height calculation for image is completed. By resizing the window,
        // Drupal triggers the method "rebuild_max_height". Check: sites/all/modules/contrib/field_slideshow/field_slideshow.js
  ...
  /**
   * DC has implemented a plugin called Optanon to comply with GDPR.
   * There is no obvious way to turn this off while the site is running locally
   * and the plugin prompts us every page load which makes development tedious.
   * This is a nasty hack to automatically click the accept cookies button as soon as the button
   * exists if the current domain is a local domain.
   */
  $(window).load(function() {
    var localDomains = [
      'dccomics.local',
    ...
    if (localDomains.includes(location.host)) {
      runAfterElementExists('#consent-modal', function ($element) {
        console.log('Automatically opted in to cookies on local env');
  ...
  var runAfterElementExists = function(jquery_selector, callback) {
    var checker = window.setInterval(function() {
      var $element = $(jquery_selector);
      if ($element.length) {
