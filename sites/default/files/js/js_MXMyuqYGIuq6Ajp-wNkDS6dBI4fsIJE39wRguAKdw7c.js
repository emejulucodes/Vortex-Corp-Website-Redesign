...
;
// Factoid handler for DC

(function($) {
  Drupal.behaviors.factoids = {
    attach: function (context, settings) {
      // Omniture related tags
      var factoidPre;
      if ($('.front').length > 0) {
        factoidPre = 'dchpff';
      } else {
      ...
      //create factoids from the view
      $('.view-factoids').once(function() {
        var factoid,
          rows = $('.views-row', this);
        for (var i = 0; i < 2; i++) {
          ...
          factoid = $(rows[ran])
            .addClass('factoid')
            .addClass('factoid-' + (i + 1));
          // If the content is text only, give it a class for styling
          if ($('img', factoid).length === 0) {
            factoid.addClass('text-only');
          }
          factoid
            .find('a')
            .attr(
              'href',
              addQuery(
                factoid.find('a').attr('href'),
                'adid=' + factoidPre + (i + 1)
              )
            );
          rows.splice(ran, 1);
          moveFactoid(factoid, i + 1);
        }
      });

      // Specific move conditions
      function moveFactoid(factoid, pos) {
        switch (pos) {
          case 1:
            // Home page Get to Know section
            if ($('.front .gtk.tall').length > 0) {
              $('.front .gtk.tall').prepend(factoid);
            }
            break;
          case 2:
            // Home page Featured section
            if ($('.front .pane-bundle-featured .pane-content').length > 0) {
              $('.pane-bundle-featured .pane-content').append(factoid);
            }
            break;
        }
      }
      ...
      // Scroll function to animate factoids
      $(window).scroll(function(event) {
        if ($('.factoid').length > 0) {
          ...
          var $b = $('.factoid-2');
          var w = $(this).height();
          if (y + w > $a.offset().top && !$a.hasClass('pop')) {
            $a.addClass('pop');
          }
          if (y + w > $b.offset().top && !$b.hasClass('pop')) {
            $b.addClass('pop');
          }
        }
      });
